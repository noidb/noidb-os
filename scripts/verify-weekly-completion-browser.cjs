...
(async () => {
  ...
  try {
    ...
    assert.equal(await reviews.locator("article").count(), 3, "Unfinished work stays active; catalog-discontinued SKU is already resolved");
    ...
    assert.equal(await blocked.count(), 0, "Already discontinued product is kept in completion history instead of recurring review");
    ...
    assert.equal(await page.getByRole("button", {name:"쿠폰·광고 등록 완료",exact:true}).isEnabled(),false,"Coupon expiry required");
    await page.getByLabel("쿠팡 쿠폰 종료일",{exact:true}).fill("2099-12-31");
    ...
    assert.equal(await reviews.locator("article").count(), 0);
    ...
    assert.equal(await reviews.locator("article").count(), 0, "Completed rows remain hidden after reload");
    assert.equal(await page.locator('[aria-labelledby="weekly-coupon-title"]').count(), 0);
    ...
    assert.match(await history.innerText(), /SKU 38256624/);
