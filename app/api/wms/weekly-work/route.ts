...
import { loadWeeklySnapshot } from "@/lib/wms/weekly-work-source";
...
export async function POST(request:NextRequest) {
  ...
  try {
    ...
    if(body.action==="analyze") {
      const period = body.period as WeeklyPeriod | undefined;
      if (!period || !/^\d{4}-\d{2}-\d{2}$/.test(period.startDate) || !/^\d{4}-\d{2}-\d{2}$/.test(period.endDate)) throw new Error("조회 기간을 확인해 주세요.");
      const existing=await readWeeklyWorkspace();
      const carryPurchaseOrders=[...new Set(existing.runs.filter(run=>!run.completedAt&&!run.reorderRequestedAt).flatMap(run=>run.snapshot.vendorItems.flatMap(item=>item.relatedPurchaseOrderNumbers)))];
      const snapshot=await loadWeeklySnapshot(body.period as WeeklyPeriod,{browserSource:body.browserSource as WeeklyBrowserSource|undefined,uploadedDatasets,carryPurchaseOrders});
      const run=await mutateWeeklyWorkspace(workspace=>{ workspace.materialSnapshot=structuredClone(snapshot); return addWeeklyRun(workspace,snapshot); });
      const { readSavedStatusList } = await import("@/lib/wms/status-list-cache");
      await readSavedStatusList(true).catch(()=>undefined);
    ...
    if(body.action==="status") {
      ...
      if(kind==="reorder") {
        ...
        if(!before.reorderRequestedAt&&before.snapshot.operationalToken!==savedWeeklyOperationalToken(await readWeeklyWorkspace()))throw new Error("분석 이후 발주·입고 자료가 변경됐습니다. 최신 자료로 다시 준비해 주세요.");
      }
      if(kind==="discontinue") {
