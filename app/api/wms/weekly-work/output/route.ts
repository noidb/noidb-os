...
export async function POST(request:NextRequest) {
  ...
  try {
    ...
    if(includesReorders) {
      ...
      if(run.snapshot.operationalToken!==savedWeeklyOperationalToken(await readWeeklyWorkspace()))throw new Error("분석 이후 발주·입고 자료가 변경됐습니다. 최신 자료로 다시 준비해 주세요.");
    }
    ...
    if(cached)output=JSON.parse(cached.toString());
      ...
      if(!includesReorders&&(body.kind==="all"||body.kind==="vendors")&&run.snapshot.operationalToken!==savedWeeklyOperationalToken(await readWeeklyWorkspace()))throw new Error("분석 이후 발주·입고 자료가 변경됐습니다. 최신 자료로 다시 준비해 주세요. 기존에 생성한 파일은 다시 받을 수 있습니다.");
      output=await buildWeeklyOutput(run,body.kind,now,advertising);
      await saveWeeklyFile(key,Buffer.from(JSON.stringify(output)));
    }
    {
      if(includesReorders && run.snapshot.operationalToken!==savedWeeklyOperationalToken(await readWeeklyWorkspace()))throw new Error("파일 생성 중 발주·입고 자료가 변경됐습니다. 최신 자료로 다시 준비해 주세요.");
