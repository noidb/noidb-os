...
async function check(waveId: string, candidates?: VendorOrderDraftLine[], expectedUpdatedAtByLineId?: Record<string, string | null>, expectedDraftUpdatedAtById?: Record<string, string | null>) {
  ...
  const routed = new Set(scope.routedVendorLineIds || []);
  const excludedLineIds = [...new Set([...exclusions.map(row => row.lineId), ...lines.filter(line => isVendorLineResolved(store.vendorOrderLines.find(saved => saved.id === line.id) || line) || routed.has(line.id)).map(line => line.id)])], excluded = new Set(excludedLineIds);
