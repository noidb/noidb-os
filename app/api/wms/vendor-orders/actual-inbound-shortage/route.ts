...
async function loadLines(pendingOnly = false) {
  const [{ orders, latestSnapshotTimeMsByPurchaseOrderNumber, snapshotConflicts }, catalog, stockedFile, workspace, pickingStore] = await Promise.all([
    ...
    const completedPairs = new Set(completedShortagePairs(workspace.runs.map(run => ({ ...run }))));
    const activeStore = { ...pickingStore, vendorOrderLines: pickingStore.vendorOrderLines.filter(line => !pickingStore.deletedVendorLineIds[line.id] && !pickingStore.deletedVendorDraftIds[line.draftId]) };
    for (const item of historicalShortageEvidence(workspace, activeStore)) {
      if (item.status === "already_resolved" || item.status === "discontinued") completedPairs.add(JSON.stringify([item.purchaseOrderNumber, item.skuId]));
    }
    const isPendingPair = (orderNo: string, skuId: string) => !completedPairs.has(JSON.stringify([orderNo.trim(), skuId.trim()]));
    ...
    if (statuses.length > 0 && events.length > 0) {
      ...
      const lines = calculation.shortagePairs.filter(item =>
        (!pendingOnly || isPendingPair(item.orderNo, item.skuId)) && !/단종/.test(catalogBySku.get(normalizeSkuId(item.skuId))?.currentStatus || "")
      ).map(item => {
    ...
    const lines = computeActualInboundShortageLines(mergedOrders, catalog.items).filter(line =>
      !routedReorders.has(JSON.stringify([line.purchaseOrderNumber, line.productCode])) && (!pendingOnly || isPendingPair(line.purchaseOrderNumber, line.productCode))
    ).map(line => ({
      ...line,
...
export async function GET(request: Request) {
  try {
    const { lines, stockedFile, snapshotConflicts, calculation } = await loadLines(new URL(request.url).searchParams.get("pending") === "1");
