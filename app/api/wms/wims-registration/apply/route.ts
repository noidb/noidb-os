...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json() as { rows?: WimsRegistrationRow[]; dryRunToken?: string; confirmation?: string; includeReviewing?: boolean };
    const includeReviewing = body.includeReviewing === true;
    if (body.confirmation !== (includeReviewing ? "WIMS 검수상태 반영" : "WIMS 검수완료 상품 연결")) return NextResponse.json({ error: "정확한 반영 확인 문구가 필요합니다." }, { status: 423 });
    ...
    return NextResponse.json(await applyWimsRegistrationAudit(body.rows, String(body.dryRunToken || ""), includeReviewing));
  } catch (error) {
