...
export default function SentReorderButton({line,onSaved,style,disabled=false}:{line:VendorOrderDraftLine;onSaved:(line:VendorOrderDraftLine)=>void;style?:CSSProperties;disabled?:boolean}){
 ...
 return <><button type="button" disabled={disabled} style={{...wmsSageButton,width:'100%',minHeight:48,fontSize:13,...style}} onClick={()=>{setOpen(true);void load();}}>미납분재발주요청</button>
 {open&&<div style={{position:'fixed',inset:0,zIndex:1200,background:'#0008',padding:12,display:'grid',placeItems:'center'}}><section role="dialog" aria-modal="true" aria-label="입고 후 미납분재발주요청" style={{width:'100%',maxWidth:440,maxHeight:'85dvh',overflowY:'auto',boxSizing:'border-box',borderRadius:14,padding:18,background:'white'}}>
 <h3>입고 후 미납분재발주요청</h3><p>{line.productName}<br/>SKU {line.skuId}</p>
 {preview&&<>{preview.linkedFromSku&&<p style={{color:'#934633'}}>직접 추가한 상품의 SKU로 저장된 미입고 자료를 찾았습니다. 아래 발주번호와 수량이 맞는지 확인해 주세요.</p>}<table style={{width:'100%',fontSize:14}}><thead><tr><th>쿠팡 발주번호</th><th>미입고 수량</th></tr></thead><tbody>{preview.rows.map(row=><tr key={row.purchaseOrderNumber}><td>{row.purchaseOrderNumber}</td><td style={{textAlign:'center'}}>{row.shortageQuantity}개</td></tr>)}</tbody></table><p><strong>미납분재발주요청 {preview.quantity}개</strong> · 거래처 주문 {line.shortageQuantity}개</p><p>제품이 입고된 것을 확인하면 기존 미납분재발주 대기에 취합하고 이 발주서 목록에서 제외합니다.</p></>}
 {error&&<p role="alert" style={{color:'#b42318'}}>{error}</p>}{busy&&<p role="status">처리 중…</p>}
 <button type="button" style={{...wmsSageButton,width:'100%',minHeight:46}} disabled={busy||!preview} onClick={()=>void save()}>입고 확인 · 미납분재발주요청</button>
