...
export default function CompleteReceivingButton({ line, onSaved, stockOnly=false, style }: { line: VendorOrderDraftLine; onSaved: (line: VendorOrderDraftLine) => void; stockOnly?:boolean; style?: CSSProperties }) {
  ...
  return <>
    <button type="button" disabled={busy} style={{...wmsPrimaryButton,minHeight:'48px',fontSize:'13px',width:'100%',...style}} onClick={()=>{setOpen(true);setStock(stockOnly||Boolean(line.isStockReplenishment));void prepare(stockOnly||Boolean(line.isStockReplenishment));}}>{stockOnly?'보충분 입고완료':'입고완료'}</button>
    {open&&<div style={{position:'fixed',inset:0,zIndex:85,background:'rgba(37,37,37,.6)',display:'flex',alignItems:'center',justifyContent:'center',padding:12}}><section role="dialog" aria-modal="true" aria-label="입고완료 확인" style={{width:'100%',maxWidth:420,maxHeight:'85dvh',overflowY:'auto',boxSizing:'border-box',background:'white',borderRadius:14,padding:18}}>
      ...
      {stockOnly?<p>이 상품은 보충분으로 입고완료 처리합니다.</p>:<label style={{display:'flex',alignItems:'center',gap:8,minHeight:44,fontSize:14}}><input type="checkbox" checked={stock} disabled={busy} onChange={e=>{setStock(e.target.checked);void prepare(e.target.checked);}} style={{width:22,height:22}}/>재고보충 주문 (미납분재발주요청 제외)</label>}
      <p style={{fontSize:13,lineHeight:1.6}}>{stock?'입고완료 후 진행 발주 목록에서 빠지고 입고이력에 남습니다.':'입고기록을 저장합니다. 미입고분에서 온 상품은 미납분재발주요청으로 이동해 취합하세요.'}</p>
