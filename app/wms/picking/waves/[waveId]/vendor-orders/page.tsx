...
export default function VendorOrdersPage({ params }: { params: { waveId: string } }) {
  ...
  async function addPendingReorder(source: VendorOrderDraftLine) {
    ...
    const now = new Date().toISOString();
    // actual-inbound-shortage 출처(쿠팡 실제 미납)는 12개 단위 올림을 절대 쓰지 않는다 — 재발주
    // 큐를 거쳐도 같은 규칙이 유지돼야 한다(2026-09-11). picking-shortage 출처는 기존 그대로 올림.
    const isActualInboundShortage = source.sourceType === "actual-inbound-shortage";
    const roundQuantity = (value: number) => isActualInboundShortage ? value : toVendorOrderQuantity(value);
    ...
    if (existing) {
      updateLine(existing.id, {
        ...
        shortageQuantity: roundQuantity((existing.actualShortageQuantity || existing.shortageQuantity) + pendingQuantity),
      ...
      setLines(prev => [...prev, {
        ...
        shortageQuantity: roundQuantity(pendingQuantity),
        receivedQuantity: 0,
