  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch("/api/wms/hanjin-upload/build-shipment-upload", {
        ...
        body: JSON.stringify({ fileBase64: trackingFileBase64, waveId, targets }),
      });
  ...
  return (
    <div>
      <p style={{ fontSize: "11px", color: wmsColors.muted, margin: "0 0 10px" }}>
        2단계에서 확인된 실제 송장번호를 반영해 Supplier Hub 쉽먼트 생성용 업로드파일을 만듭니다.
        송장번호가 없는(매칭 실패) 행은 포함하지 않습니다. 외부 사이트에는 자동 업로드하지
        않습니다 — 파일만 만들어집니다.
      </p>
      ...
      {!trackingFileBase64 && (
        <p style={{ fontSize: "12px", color: wmsColors.warnText, background: wmsColors.warnSoft, borderRadius: "8px", padding: "8px 10px", margin: "0 0 10px" }}>
          2단계에서 송장번호 입력 완료 파일을 먼저 업로드해야 실행할 수 있습니다.
        </p>
      )}
      ...
      <button
        ...
        disabled={generating || !trackingFileBase64}
        style={{ ...wmsGreenDarkButton, width: "100%", opacity: generating || !trackingFileBase64 ? 0.5 : 1 }}
      >
