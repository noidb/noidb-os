...
export default function ActualInboundShortagePage() {
  ...
  async function reload() {
    ...
    try {
      const [response, allLines] = await Promise.all([
        fetch("/api/wms/vendor-orders/actual-inbound-shortage", { cache: "no-store" }),
      ...
      setExistingKeys(new Set(
        allLines
          ...
          .map(line => keyOf(line.relatedPurchaseOrderNumbers[0] || "", line.skuId))
      ));
      setSelected(new Set());
      setClassifications({});
      setFailures({});
  ...
  return (
    <main style={{ maxWidth: WMS_MOBILE_WIDTH, margin: "0 auto", padding: "12px 12px calc(12px + env(safe-area-inset-bottom))", fontFamily: "sans-serif", background: wmsColors.background, color: wmsColors.ink, minHeight: "100vh" }}>
      ...
      <h1 style={{ fontSize: "20px", margin: "10px 0 4px" }}>실제 미납 처리</h1>
