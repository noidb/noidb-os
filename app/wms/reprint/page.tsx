...
export default function WmsReprintCenterPage() {
  ...
  return <main className="shell wms-work-center-shell" style={{ fontFamily: "sans-serif", color: wmsColors.ink, paddingBottom: "40px" }}>
    ...
    <form onSubmit={search} style={{ ...wmsOuterCard, padding: "14px", display: "grid", gap: "8px" }}>
      <label style={{ fontSize: "12px", fontWeight: 800 }}>바코드 검색</label>
      <input value={query} onChange={event => setQuery(event.target.value)} placeholder="SKU ID, 바코드, 발주번호, 상품명" style={{ width: "100%", minHeight: "48px", boxSizing: "border-box", border: `1px solid ${wmsColors.borderStrong}`, borderRadius: "10px", padding: "0 12px", fontSize: "16px" }} />
    ...
    <section style={{ display: "grid", gap: "9px", marginTop: "12px" }}>
      {results.map(result => {
        ...
        return <article key={key} style={{ ...wmsOuterCard, padding: "12px" }}>
          <strong style={{ display: "block", fontSize: "14px", lineHeight: 1.45 }}>{result.productName}</strong>
