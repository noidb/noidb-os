...
export default function OutboundWorkCenter() {
  ...
  async function saveState() {
    ...
    try {
      ...
      const response = await fetch("/api/wms/work-center", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ waveId: change.work.id, status: change.status, expectedUpdatedAt: change.work.state?.updatedAt || null, expectedWorkUpdatedAt: change.work.updatedAt, confirmed: true }) });
      const result = await response.json();
  ...
  const active = overview?.works.filter(work => !work.fullyCompletedElsewhere && !work.completedAt && (!work.state || work.state.status === "active")) || [];
  const filed = overview?.works.filter(work => work.fullyCompletedElsewhere || work.completedAt || (work.state && work.state.status !== "active")) || [];
  const next = active[0];
  const statusLabel = (status: OutboundWorkState["status"]) => status === "active" ? "작업 중으로 복원" : status === "archived" ? "보관" : "출고완료";
  const renderWork = (work: OutboundWorkSummary) => <article className={`${styles.work} ${work.id === next?.id ? styles.nextWork : ""}`} key={work.id} data-testid="outbound-work">
    <div className={styles.row}><h3>{work.title}</h3><span className={styles.badge}>{work.fullyCompletedElsewhere ? "다른 작업에서 출고완료" : work.state?.status === "archived" ? "보관" : work.completedAt ? "출고완료" : "작업 중"}</span></div>
    ...
    {!work.fullyCompletedElsewhere && <p className={styles.nextLabel}>다음 작업: {work.nextLabel}</p>}
    {!work.fullyCompletedElsewhere && <a className={styles.primary} href={work.nextHref}>{work.nextHref.includes("/packing") ? "출고작업 계속하기 →" : work.id === next?.id ? "서류·출력세트 확인/수정 →" : `${work.nextLabel} →`}</a>}
    ...
    {!work.fullyCompletedElsewhere && !work.completedAt && (!work.state || work.state.status === "active") && <button type="button" className={styles.completeButton} disabled={saving} onClick={() => setChange({ work, status: "completed" })}>출고완료</button>}
    {(work.completedAt || (work.state && work.state.status !== "active")) && <details className={styles.details}>
  ...
  return <main className={`shell wms-work-center-shell ${styles.shell}`}>
    ...
    <div className={styles.tasks}>
      ...
      <section className={styles.task}><h2>주간 업무</h2><p>기간을 정해 쿠폰·거래처 발주·단종 신청을 한 번에 준비합니다.</p><a className={styles.secondary} href="/wms/inbound">주간 업무 시작</a></section>
    ...
    <details className={styles.section}><summary>완료·보관 {overview ? `(${filed.length})` : ""}</summary><p className={styles.muted}>출고완료한 작업과 보관한 작업입니다. 필요할 때 재출력하거나 작업 중으로 복원할 수 있습니다.</p><div className={styles.workGrid}>{filed.map(renderWork)}</div></details>
    ...
    {change && <div className={styles.overlay}><section role="dialog" aria-modal="true" aria-labelledby="work-state-title" className={styles.dialog}>
      <h2 id="work-state-title">{statusLabel(change.status)} 확인</h2><p>{change.work.title}</p><p>발주 {change.work.purchaseOrderCount}건 · SKU {change.work.skuCount}개</p><p>변경: {change.work.state?.status === "archived" ? "보관" : change.work.completedAt ? "출고완료" : "작업 중"} → {statusLabel(change.status)}</p>{change.status === "completed" ? <p>이 작업의 모든 택배를 실제로 인계했다면 출고완료해 주세요. 오늘 할 일에서 사라지고 완료·보관에서 다시 볼 수 있습니다.</p> : <p>작업 목록의 표시 상태를 변경합니다.</p>}<p className={styles.muted}>기존 서류·수량·운송장과 작업 이력은 보존됩니다.</p>
