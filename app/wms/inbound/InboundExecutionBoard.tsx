...
type Summary = { expectedDate: string; center: string; purchaseOrderCount: number; skuCount: number; quantity: number };
type Step = { title: string; action: string; note: string; href?: string };

const STORAGE_KEY = "noidb-inbound-execution-board-v1";
const steps: Step[] = [
  { title: "Supplier Hub 발주확정", action: "Supplier Hub 발주리스트 열기", note: "입고예정일(EDD)로 거래처확인요청을 확인해 ‘발주서 업로드 양식’을 받고, 원본 파일 PO_FOR_CONFIRM*.xlsx는 G:\\내 드라이브\\쿠팡데이터\\발주서업로드완성에 저장합니다.", href: "https://supplier.coupang.com" },
...
export default function InboundExecutionBoard({ summaries }: { summaries: Summary[] }) {
  ...
  return <section aria-label="입고 실행판" style={{ margin: "22px 0", padding: 18, border: "2px solid #b9d5bd", borderRadius: 12, background: "#f7fbf5" }}>
    ...
    <div style={{ overflowX: "auto", marginTop: 16 }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><caption style={{ textAlign: "left", paddingBottom: 8, fontWeight: 700 }}>현재 발주 원본 기준 입고 대상</caption><thead><tr>{["입고예정일", "센터", "발주 건수", "SKU", "수량"].map(label => <th key={label} scope="col" style={{ textAlign: "left", padding: 8, background: "#e8f2e7", borderBottom: "1px solid #cfe0cf" }}>{label}</th>)}</tr></thead><tbody>{summaries.length ? summaries.map(row => <tr key={`${row.expectedDate}-${row.center}`}><td style={{ padding: 8, borderBottom: "1px solid #e3ece2" }}>{row.expectedDate}</td><td style={{ padding: 8, borderBottom: "1px solid #e3ece2" }}>{row.center}</td><td style={{ padding: 8, borderBottom: "1px solid #e3ece2" }}>{row.purchaseOrderCount.toLocaleString()}건</td><td style={{ padding: 8, borderBottom: "1px solid #e3ece2" }}>{row.skuCount.toLocaleString()}개</td><td style={{ padding: 8, borderBottom: "1px solid #e3ece2" }}>{row.quantity.toLocaleString()}개</td></tr>) : <tr><td colSpan={5} style={{ padding: 8 }}>불러온 발주 원본이 없습니다. 1단계에서 발주 원본을 가져와 주세요.</td></tr>}</tbody></table></div>
