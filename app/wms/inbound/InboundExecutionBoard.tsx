...
const steps: Step[] = [
  ...
  { title: "한진 송장파일 준비", action: "", note: "발주서리스트 원본 파일의 물류센터 주소·전화·우편번호로 1발주 1행 한진택배_업로드_YYYYMMDD_HHMMSS.xlsx를 만듭니다. 저장: G:\\내 드라이브\\쿠팡데이터\\한진택배 송장파일." },
  { title: "한진 n-Focus 등록·출력·세부내역 저장", action: "한진 n-Focus 열기", note: "쿠팡 [고정형]·운임 S·오류체크 후 직접 출력하고, 재출력 및 출고관리에서 세부내역 엑셀을 내려받아 재출력_세부내역_YYYYMMDD_HHMMSS.xlsx로 저장합니다. 위치: G:\\내 드라이브\\쿠팡데이터\\한진택배 송장파일.", href: "https://focus.hanjin.com" },
...
export default function InboundExecutionBoard({ summaries }: { summaries: Summary[] }) {
  ...
  return <section aria-label="입고 실행판" style={{ margin: "22px 0", padding: 18, border: "2px solid #b9d5bd", borderRadius: 12, background: "#f7fbf5" }}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "baseline", flexWrap: "wrap" }}>
      ...
      <strong>{done}/{steps.length} 단계 완료</strong>
    </div>
    ...
    <ol style={{ paddingLeft: 22, margin: "18px 0 0" }}>{steps.map((step, index) => <li key={step.title} style={{ margin: "12px 0", padding: 10, borderRadius: 8, background: completed[index] ? "#e7f4e8" : "#fff" }}>
      ...
      {step.action ? <div style={{ margin: "8px 0 0 25px" }}>{step.href ? <a href={step.href} target={step.href.startsWith("http") ? "_blank" : undefined} rel={step.href.startsWith("http") ? "noreferrer" : undefined}>{step.action} ↗</a> : <span style={{ color: "#526158" }}>{step.action}</span>}</div> : null}
    </li>)}</ol>
    <button type="button" onClick={() => setCompleted(steps.map(() => false))} disabled={!done} style={{ padding: "8px 12px" }}>진행상태 초기화</button>
    {done === steps.length ? <p role="status" style={{ marginBottom: 0, color: "#176b32", fontWeight: 700 }}>전체 물류작업 {steps.length}단계 완료 기록됨</p> : null}
  </section>;
}
