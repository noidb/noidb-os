...
export default function QuickDetailComposer() {
  ...
  async function chooseFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const sections = await splitDetailPage(source, headerUrl);
      setSource(source);
  ...
  async function chooseHeader(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setProgress(0);
      if (source) {
        const sections = await splitDetailPage(source, nextHeader);
        setOriginalSections(sections);
        setMessage(`상단 로고를 바꿨습니다. ${sections.length}장의 사진을 새 브랜드 상세페이지로 만들 수 있습니다.`);
      } else {
  ...
  async function useNoidbHeader() {
    ...
    try {
      ...
      setResult(null);
      setProgress(0);
      if (source) setOriginalSections(await splitDetailPage(source, HEADER_URL));
  ...
  async function create() {
    ...
    try {
      ...
      const composed = await composeQuickDetailPage(headerUrl, completed);
      setResult(composed);
