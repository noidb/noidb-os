...
async function rememberStatusRecords(records: StatusRequestRecord[]) {
  await mutateWeeklyWorkspace(workspace => {
    ...
    workspace.statusListSnapshot.requests = [...workspace.statusListSnapshot.requests.filter(record => !ids.has(record.id)), ...records];
  });
}
...
/** 단종/해제 후보를 제품DB 상태를 바꾸지 않고 처리대기 목록에 넣는다. */
export async function queueStatusCandidate(input: {
  ...
  const [requests, product, workspace] = await Promise.all([listStatusRequests(), readProductSnapshot(skuId), readWeeklyWorkspace()]);
  ...
  if (pending.length) {
    ...
    const linked = [...new Set([record.purchaseOrderNumber, input.purchaseOrderNumber || ""].flatMap(value => value.split(/[,/\s]+/).filter(Boolean)))].join(" / ");
  ...
  const record: StatusRequestRecord = {
    id: makeId("status"), skuId: normalizeSkuId(skuId), modelSku: String(product.values[headerIndex("모델SKU")] ?? "").trim(),
    productName: display.name, optionLabel: display.option, currentStatus, requestType: input.requestType, requestedAt: new Date().toISOString(), supplyHubStatus: "처리대기",
    completedAt: "", requester: operator, processor: "", previousStatus: currentStatus,
    productLink: String(product.values[product.headers.indexOf("제품링크")] ?? "").trim(), purchaseOrderNumber: String(input.purchaseOrderNumber || "").trim(), sheetRow: 0,
  };
  await ensureHiddenSheet(STATUS_REQUEST_SHEET, [...STATUS_REQUEST_HEADERS]);
  await appendSheetRow(STATUS_REQUEST_SHEET, [record.id, record.skuId, record.modelSku, record.productName, record.optionLabel, record.currentStatus, record.requestType, record.requestedAt, record.supplyHubStatus, "", record.requester, "", currentStatus || EMPTY_STATUS_TOKEN, record.productLink, record.purchaseOrderNumber]);
  await rememberStatusRecords([record]);
  return record;
}

/** 생성한 단종/해제 파일의 이력만 남긴다. 요청의 처리 상태나 제품DB는 바꾸지 않는다. */
export async function recordStatusFileGeneration(input: {
  ...
  const matched = requestIds.map(id => requestById.get(id)).filter((item): item is StatusRequestRecord => Boolean(item));
  ...
  const record: StatusFileGenerationRecord = {
    ...
    xlsxFileName, pdfFileName, operator: String(input.operator || "자동").trim() || "자동",
  };
  ...
  await mutateWeeklyWorkspace(workspace => {
    if (workspace.statusListSnapshot) workspace.statusListSnapshot.generations.push(record);
  });
  return record;
}
