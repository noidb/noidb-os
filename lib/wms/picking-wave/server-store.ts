...
import { discardVendorOrder } from "../vendor-order/discard-order";
import { transferSentVendorLine } from "../vendor-order/sent-vendor-transfer";
import { isVendorLineResolved } from "../vendor-order/receiving-state";
...
function normalizeSnapshot(value: unknown): PickingWaveStoreSnapshot {
  ...
  return {
    ...
    ...(raw.discardedVendorOrders && typeof raw.discardedVendorOrders === "object" ? { discardedVendorOrders: raw.discardedVendorOrders } : {}),
    ...(raw.discardedVendorLines && typeof raw.discardedVendorLines === "object" ? { discardedVendorLines: raw.discardedVendorLines } : {}),
    ...
    ...(Array.isArray(raw.supplierHubOrderStatuses) ? { supplierHubOrderStatuses: raw.supplierHubOrderStatuses } : {}),
    ...(Array.isArray(raw.supplierHubInboundEvents) ? { supplierHubInboundEvents: raw.supplierHubInboundEvents } : {}),
...
export function applyPickingWaveStoreMutation(current: PickingWaveStoreSnapshot, mutation: PickingWaveStoreMutation, completionScope?: VendorOrderCompletionScope, catalogItems?: Parameters<typeof resolveVendorOrderCatalog>[0]["catalogItems"]): PickingWaveStoreSnapshot {
  ...
  if (mutation.action === "consolidateVendorOrders") {
  ...
  } else if (mutation.action === "transferSentVendorLine") {
    return transferSentVendorLine(current, mutation);
  } else if (mutation.action === "resolveSentVendorLine") {
    const source = next.vendorOrderLines.find(line => line.id === mutation.lineId);
    if (source?.sentResolution?.kind === mutation.kind && source.sentResolution.destinationId === mutation.destinationId) return current;
    if (!source || source.updatedAt !== mutation.expectedUpdatedAt || isVendorLineResolved(source) || next.deletedVendorLineIds[source.id] || next.deletedVendorDraftIds[source.draftId] || !next.vendorOrderDrafts.some(d => d.id === source.draftId && d.status === "sent")) throw new Error("원본 발주 품목이 변경됐습니다. 이동한 목록과 최신 발주서를 확인해 주세요.");
    next.vendorOrderLines = next.vendorOrderLines.map(line => line.id === source.id ? { ...line, sentResolution: { kind: mutation.kind, destinationId: mutation.destinationId, at: mutation.now }, updatedAt: mutation.now } : line);
  } else if (mutation.action === "setSentVendorDelay") {
    const source=next.vendorOrderLines.find(line=>line.id===mutation.lineId);
    if(!source||isVendorLineResolved(source)||next.deletedVendorLineIds[source.id]||next.deletedVendorDraftIds[source.draftId]||!next.vendorOrderDrafts.some(d=>d.id===source.draftId&&d.status==="sent"))throw new Error("입고지연으로 남길 전송완료 상품을 다시 확인해 주세요.");
    const active=Boolean(source.receivingDelayedAt&&!source.receivingDelayReleasedAt);
    ...
    if (!lineBefore || !owner || !["sent", "approved"].includes(owner.status) || lineBefore.orderExclusion || lineBefore.vendorTransfer || lineBefore.sentResolution || next.deletedVendorLineIds[lineBefore.id] || next.deletedVendorDraftIds[lineBefore.draftId]) throw new Error("입고완료할 전송·승인 발주 품목을 다시 확인해 주세요.");
    const saved = completeReceivingLine(lineBefore, mutation.before, mutation.isStockReplenishment, mutation.now);
    if (saved === lineBefore) return current;
  ...
  } else if (mutation.action === "discardVendorOrder") {
    if (!discardVendorOrder(next, mutation)) return current;
    ...
    for (const line of selected) {
      ...
      const reason = getVendorLineDeletionBlockReason(line, draft, true);
    ...
    next.discardedVendorLines = { ...next.discardedVendorLines };
    for (const line of selected) {
      next.discardedVendorLines[line.id] = { line: structuredClone(line), draft: structuredClone(drafts.get(line.draftId)!), deletedAt: mutation.deletedAt };
      next.deletedVendorLineIds[line.id] = mutation.deletedAt;
