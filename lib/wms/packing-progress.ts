...
export interface PackingProgress {
  generationKey: string; manifestKey: string; rows: PackingRow[]; checkedKeys: string[]; updatedAt: string; dispatchedAt?: string;
}
...
export function nextPackingProgress(wave: PickingWave, items: PickingWaveItem[], prior: PackingProgress | undefined, input: { generationKey: string; rows: PackingRow[]; checkedKeys: string[]; expectedUpdatedAt: string | null; dispatched: boolean }, now: string): PackingProgress {
  ...
  return { generationKey: input.generationKey, manifestKey, rows: input.rows, checkedKeys: input.checkedKeys, updatedAt: now, ...(input.dispatched ? { dispatchedAt: now } : {}) };
}
