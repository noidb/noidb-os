...
function supplierHubSnapshotAvailable(store: PickingWaveStoreSnapshot): boolean {
  return Array.isArray(store.supplierHubOrderStatuses) || Array.isArray(store.supplierHubInboundEvents);
}

function buildSupplierHubSnapshot(input: BuildInput): WeeklySnapshot {
  const calculation = calculateSupplierHubShortages({
    statuses: input.pickingStore.supplierHubOrderStatuses || [],
    events: input.pickingStore.supplierHubInboundEvents || [],
  ...
  for (const item of input.catalogItems) {
    ...
    if (skuId && !catalog.has(skuId)) catalog.set(skuId, item);
  }
  const groups = new Map<string, typeof calculation.shortagePairs>();
  for (const item of calculation.shortagePairs) groups.set(item.skuId, [...(groups.get(item.skuId) || []), item]);
  const vendorItems: WeeklyVendorItem[] = [...groups].map(([skuId, items]) => {
    const product = catalog.get(skuId);
    const details = items.map(item => ({ purchaseOrderNumber: item.orderNo, confirmedQuantity: item.confirmedQuantity,
      receivedQuantity: item.receivedQuantity, shortageQuantity: item.shortageQuantity }));
    const shortageQuantity = details.reduce((sum, item) => sum + item.shortageQuantity, 0);
    return { skuId, productName: items.find(item => item.skuName)?.skuName || product?.productName || "", productLink: product?.productLink || "",
      vendorName: product?.vendorName || "", imageUrl: product?.imageUrl || "", optionLabel: product?.optionLabel || "", modelName: product?.modelName || "", barcode: product?.barcode || "",
      shortageQuantity, openOrderQuantity: 0, suggestedQuantity: shortageQuantity,
      confirmedQuantity: details.reduce((sum, item) => sum + item.confirmedQuantity, 0), receivedQuantity: details.reduce((sum, item) => sum + item.receivedQuantity, 0),
      shortageDetails: details.sort((a, b) => a.purchaseOrderNumber.localeCompare(b.purchaseOrderNumber)),
      relatedPurchaseOrderNumbers: [...new Set(details.map(item => item.purchaseOrderNumber))].sort(), issues: [], discontinued: /단종/.test(product?.currentStatus || "") };
  });
  const unresolvedBySku = new Map<string, typeof calculation.unresolvedPairs>();
  for (const item of calculation.unresolvedPairs) unresolvedBySku.set(item.skuId, [...(unresolvedBySku.get(item.skuId) || []), item]);
  const unresolvedItems: WeeklyUnresolvedItem[] = [...unresolvedBySku].map(([skuId, items]) => ({ skuId,
    productName: items.find(item => item.skuName)?.skuName || catalog.get(skuId)?.productName || "",
    relatedPurchaseOrderNumbers: [...new Set(items.map(item => item.orderNo))].sort(),
    issues: [...new Set(items.map(item => `발주 ${item.orderNo} · SKU ${item.skuId}: ${item.issue}`))],
  }));
  const warnings = [
    `Supplier Hub 정산완료 대상 ${calculation.exactSettledCount}건에서 발주번호+SKU별로 계산했습니다.`,
    `매입용 발주서 ${calculation.excludedPurchaseTypeCount}건은 계산에서 제외했습니다.`,
    ...(calculation.missingSourceOrderNumbers.length ? [`원본 발주수량이 없는 발주번호 ${calculation.missingSourceOrderNumbers.length}건은 계산불가로 남겼습니다.`] : []),
    ...(calculation.duplicateInboundEventCount ? [`동일 입고상세 이벤트 ${calculation.duplicateInboundEventCount}건은 중복 계산하지 않았습니다.`] : []),
  ];
  const eventDates = (input.pickingStore.supplierHubInboundEvents || []).map(event => clean(event.inboundDate).slice(0, 10)).filter(value => /^\d{4}-\d{2}-\d{2}$/.test(value)).sort();
  const sourceToken = hash({ version: 1, calculation, operationalToken: weeklyOperationalToken(input) });
  return { id: `WEEKLY-${sourceToken.slice(0, 20)}`, rulesVersion: WEEKLY_RULES_VERSION, sourceToken, operationalToken: weeklyOperationalToken(input),
    createdAt: input.now || new Date().toISOString(), period: { ...input.period },
    source: { files: ["supplier-hub-store"], firstActualDate: eventDates[0] || "", latestActualDate: eventDates.at(-1) || "", eventCount: calculation.uniqueInboundEventCount,
      duplicateCount: calculation.duplicateInboundEventCount, selectedEventCount: calculation.uniqueInboundEventCount, mode: input.mode }, couponItems: [], couponReceiptKeys: {},
    vendorItems: vendorItems.sort((a, b) => a.skuId.localeCompare(b.skuId)), unresolvedItems: unresolvedItems.sort((a, b) => a.skuId.localeCompare(b.skuId)),
    warnings, blockers: [] };
}
...
export function buildWeeklySnapshot(input: BuildInput): WeeklySnapshot {
  ...
  if (supplierHubSnapshotAvailable(pickingStore)) return buildSupplierHubSnapshot(input);
  ...
  for (const purchase of byPoSku.values()) {
    ...
    if (purchase.sourceReceived !== null && purchase.sourceReceived > total) {
      problem(purchase.sku, `발주 ${purchase.po} · SKU ${purchase.sku}: 발주서에 이미 입고 ${purchase.sourceReceived}개가 표시되지만 입고상세 누적수량은 ${total}개라 미납수량을 확정할 수 없습니다.`);
    }
