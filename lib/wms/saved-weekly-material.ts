...
export function savedWeeklyMaterial(workspace: WeeklyWorkspace, skuId: string, purchaseOrders: string[]): { snapshot: WeeklySnapshot; item: WeeklyVendorItem } {
  ...
  const snapshots = [workspace.materialSnapshot, ...workspace.runs.filter(run=>!run.id.startsWith("TRANSFER-") || Boolean(run.actualInboundRoute)).map(run => run.snapshot)].filter((snapshot): snapshot is WeeklySnapshot => Boolean(snapshot)).sort((a,b) => b.createdAt.localeCompare(a.createdAt));
  ...
  const selectedSources = pos.map(po => snapshots.find(snapshot => snapshot.vendorItems.some(item => item.skuId === skuId && item.shortageDetails?.some(detail => detail.purchaseOrderNumber === po))));
  const snapshot = selectedSources[0];
  const original = snapshot?.vendorItems.find(item => item.skuId === skuId);
  const joinedDetails = selectedSources.flatMap((source, index) => source?.vendorItems.find(item => item.skuId === skuId)?.shortageDetails?.filter(d => d.purchaseOrderNumber === pos[index]) || []);
  const item = original && { ...original, shortageDetails: joinedDetails };
  if (selectedSources.some(source => !source || source.blockers.length)) throw new Error("원발주별 저장 근거를 모두 확인하지 못했습니다.");
