...
async function readLocalDatasets(directory: string): Promise<InboundImportDataset[]> {
  const names = (await readdir(directory)).filter(name => /\.xlsx$/i.test(name)).sort();
  const results: InboundImportDataset[] = [];
  for (const name of names) {
    const filePath = path.join(directory, name);
    const info = await stat(filePath);
    results.push(await cached("weekly-inbound-local-v1", [filePath, info.mtimeMs, info.size], async () => {
      const value = await readInboundWorkbook(await readFile(filePath), name);
      return { value, contentHash: value.fingerprint, purchaseOrders: [...new Set(value.items.map(item => item.po))], actualDates: [...new Set(value.items.map(item => item.actualDate))] };
    }, validDataset));
  }
  return results;
...
async function readInboundDatasets(): Promise<InboundImportDataset[]> {
  try { return await readDriveDatasets(); }
  catch (driveError) {
    const localDirectory = process.env.WMS_WEEKLY_INBOUND_SOURCE_DIR?.trim();
    if (!localDirectory) throw driveError;
    return readLocalDatasets(localDirectory);
  }
}
...
export async function loadWeeklySnapshot(period: WeeklyPeriod, options?: { browserSource?: WeeklyBrowserSource; uploadedDatasets?: InboundImportDataset[]; carryPurchaseOrders?: string[] }): Promise<WeeklySnapshot> {
  validateWeeklyPeriod(period);
  ...
  const [drive, history, purchases, catalog, picking, workspace, manifest] = await Promise.allSettled([
    readInboundDatasets(), retryWeeklySourceRead(() => fetchSheetRows("_입고요약")), retryWeeklySourceRead(() => fetchSheetRows("_발주이력")), retryWeeklySourceRead(fetchProductCatalog), readPickingWaveStore(), readWeeklyWorkspace(), readWeeklyPurchaseManifest(),
  ]);
  const pickingStore = picking.status === "fulfilled" ? picking.value : emptyPickingWaveStoreSnapshot();
  const supplierHubStoreAvailable = Array.isArray(pickingStore.supplierHubOrderStatuses) || Array.isArray(pickingStore.supplierHubInboundEvents);
  if (drive.status === "rejected" && !primary.length && !supplierHubStoreAvailable) throw new Error("입고상세내역 파일을 읽지 못했습니다. Drive 연결을 확인하거나 서플라이 허브 자료를 연결해 주세요.");
  ...
  if (!datasets.length && !supplierHubStoreAvailable) throw new Error("분석할 입고상세내역이 없습니다. 서플라이 허브 자료를 연결하거나 입고상세내역 파일을 추가해 주세요.");
  ...
  const carryPurchaseOrders = options?.carryPurchaseOrders || [];
  const supplierHubOrderNumbers = supplierHubStoreAvailable ? [
    ...(pickingStore.supplierHubOrderStatuses || []).filter(status => status.purchaseType !== "매입용" && status.settlementStatus === "정산완료").map(status => status.orderNo),
    ...(pickingStore.supplierHubInboundEvents || []).map(event => event.orderNo),
  ] : [];
  const targetPos = [...new Set([...datasets.flatMap(dataset => dataset.items.filter(item => item.kind === "inbound" && item.totalInbound > 0
    && item.actualDate >= period.startDate && item.actualDate <= period.endDate).map(item => item.po)), ...supplierHubOrderNumbers, ...carryPurchaseOrders])].sort();
  ...
  if (!supplierHubStoreAvailable) {
    ...
    if (purchases.status === "rejected") issues.push("확정 발주수량 원본을 읽지 못해 계산불가로 남겼습니다.");
    if (catalog.status === "rejected" || (catalog.status === "fulfilled" && !catalog.value.configured)) issues.push("제품DB를 읽지 못해 상품 부가정보를 표시하지 못했습니다.");
  }
  ...
  const receiptSourceIncomplete = !supplierHubStoreAvailable && (drive.status === "rejected" || history.status === "rejected" || purchases.status === "rejected" || purchaseFileFailure);
  ...
  } else if (issues.length && !supplierHubStoreAvailable) snapshot.vendorItems = snapshot.vendorItems.map(item => ({ ...item, suggestedQuantity: 0, issues: [...new Set([...item.issues, ...issues])] }));
  const failedChannels = supplierHubStoreAvailable
    ? [purchases.status === "rejected" ? "purchases" : "", catalog.status === "rejected" || (catalog.status === "fulfilled" && !catalog.value.configured) ? "catalog" : ""].filter(Boolean)
    : [drive.status === "rejected" ? "drive" : "", history.status === "rejected" ? "history" : "", purchases.status === "rejected" ? "purchases" : "",
