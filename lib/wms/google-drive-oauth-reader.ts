...
export async function listOAuthDriveFolderFiles(folderId: string): Promise<OAuthDriveFileInfo[]> {
  ...
  let pageToken = "";
  do {
    const params = new URLSearchParams({ q, fields: "nextPageToken,files(id,name,mimeType,modifiedTime,size,md5Checksum)", orderBy: "modifiedTime desc,name", pageSize: "1000", spaces: "drive" });
