...
export function weeklyReorderRows(run: WeeklyRun): WeeklyReorderRow[] {
  const excluded = new Set([...(run.reorderPreviouslyRequestedLines || []), ...(run.reorderQueuePartialRequestedAt ? run.reorderRequestedLines || [] : [])].map(reorderPair));
...
function otherWeeklyReorderRequests(workspace: WeeklyWorkspace, ownRunId: string): ReorderLine[] {
  ...
  for (const run of workspace.runs) if (run.id !== ownRunId && (run.reorderRequestedAt || run.reorderQueuePartialRequestedAt)) {
...
export function assertWeeklyReviewEligibility(workspace: WeeklyWorkspace, run: WeeklyRun): void {
  ...
  // A routed status request may reopen work only against the explicitly saved,
  // non-discontinued source that the routing service validated.
  const statusTransfer = Object.values(workspace.workTransfers || {}).some(move => move.source === "status" && move.runId === run.id);
  const reopened = new Set(statusTransfer && workspace.materialSnapshot?.operationalToken === run.snapshot.operationalToken
    ? run.snapshot.vendorItems.filter(item => !item.discontinued && workspace.materialSnapshot?.vendorItems.some(saved => saved.skuId === item.skuId && !saved.discontinued)).map(item=>item.skuId) : []);
  const current = { ...run, previouslyDiscontinuedSkuIds: [...new Set([...(run.previouslyDiscontinuedSkuIds || []), ...otherDiscontinued])].filter(skuId=>!reopened.has(skuId)), reorderPreviouslyRequestedLines: otherWeeklyReorderRequests(workspace, run.id) };
