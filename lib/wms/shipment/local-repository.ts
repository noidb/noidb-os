...
function write(all: readonly Shipment[]): void {
...
export class LocalShipmentRepository implements ShipmentRepository {
  async listShipments(): Promise<Shipment[]> {
    return readStrict().sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  }

  async getShipment(shipmentId: string): Promise<Shipment | null> {
    return readStrict().find(shipment => shipment.id === shipmentId) ?? null;
  }
  ...
  async createFromPreviews(previews: readonly ShipmentSplitPreview[], now = new Date().toISOString()): Promise<Shipment[]> {
    ...
    const result = createShipmentsInState(readStrict(), previews, now);
    write(result.all);
    return result.created;
  }

  async renameShipment(shipmentId: string, name: string): Promise<Shipment> {
    const result = renameShipmentInState(readStrict(), shipmentId, name, new Date().toISOString());
    write(result.all);
    return result.updated;
  }

  async updateShipmentStatus(shipmentId: string, status: ShipmentStatus): Promise<Shipment> {
    const result = updateShipmentStatusInState(readStrict(), shipmentId, status, new Date().toISOString());
    write(result.all);
    return result.updated;
  }

  async updateShipmentGeneration(shipmentId: string, generation: ShipmentOutputGeneration): Promise<Shipment> {
    const result = updateShipmentGenerationInState(readStrict(), shipmentId, generation, new Date().toISOString());
    write(result.all);
    return result.updated;
  }
  ...
  async deleteShipment(shipmentId: string): Promise<void> {
    ...
    write(deleteShipmentFromState(readStrict(), shipmentId));
  }
}
